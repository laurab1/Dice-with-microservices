...
def test_roll_dice_standard(client, auth, templates):
    ...
    # 5 standard dice
    reply = client.get('/roll_dice?diceset=standard&dicenum=5', follow_redirects=True)
    assert reply.status_code == 200
    template_capture = templates[-1]['dice']
    assert len(template_capture) == 5

    # 4 standard dice
    reply = client.get('/roll_dice?diceset=standard&dicenum=4', follow_redirects=True)
    assert reply.status_code == 200
    template_capture = templates[-1]['dice']
